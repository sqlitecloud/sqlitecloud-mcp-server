  ...
  private initializeServer(): McpServer {
    return new McpServer(
      ...
      },
      {
        ...
        instructions: `
        Execute queries and manage SQLite databases on the cloud.
        Users can have multiple SQLite databases.
        The database can be preselected in the connection string or selected with a SQLite Cloud command.
        `,
      }
    );
  }
  ...
  private setupServer(): void {
    this.mcpServer.tool(
      ...
      async ({ query }, extra) => {
        ...
        const results = await this.getTransport(extra.sessionId).executeQuery(
          query
        );
      ...
      async ({}, extra) => {
        try {
          ...
          const documentationUrl =
            "https://raw.githubusercontent.com/sqlitecloud/docs/refs/heads/main/sqlite-cloud/reference/general-commands.mdx";
    ...
    this.mcpServer.tool(
      ...
      "Execute on SQLite Cloud only commands listed in the `list-commands` tool. You can use the `list-commands` tool to see the available commands.",
      {
        command: z
          .string()
          .describe("SQLite Cloud available command to execute"),
      },
      async ({ command }, extra) => {
        if (!extra.sessionId) {
          throw new Error("Session ID is required");
    ...
    this.mcpServer.tool(
      ...
      "Returns a rowset with the slowest queries performed on the connected this.mcpServer. Supports filtering with GROUPID, DATABASE, GROUPED, and NODE options.",
      {
        groupId: z
          .string()
          .optional()
          .describe("Group ID to filter the results"),
        database: z
          .string()
          .optional()
          .describe("Database name to filter the results"),
        grouped: z
          .boolean()
          .optional()
          .describe("Whether to group the slowest queries"),
        node: z
          .string()
          .optional()
          .describe(
            "Node ID to execute the command on a specific cluster node"
          ),
      },
    ...
    this.mcpServer.tool(
      ...
      "Gathers information about the indexes used in the query plan of a query execution.",
      {
        ...
        node: z
          ...
          .optional()
          .describe(
            "SQLite Cloud Node ID to execute the command on a specific cluster node"
          ),
      },
